...
public class Tester {
    ...
    public static void Test(Tests test, String singleTestName, Boolean verbose, Boolean fullOutput, Boolean stopOnFirstFail, float timeLimitMultiplier) throws IOException {
        ...
        if(test == Tests.All) {
            for (Map.Entry<Tests, Consumer<String[]>> entry : tests.entrySet()) {
                Tester.Test(entry.getKey().name(), entry.getValue(), singleTestName, verbose, fullOutput, stopOnFirstFail, timeLimitMultiplier);
            }
        }
        else {
            Tester.Test(test.name(), tests.get(test), singleTestName, verbose, fullOutput, stopOnFirstFail, timeLimitMultiplier);
        }
    }
    ...
    private static void Test(String name, Consumer<String[]> f, String singleTestName, Boolean verbose, Boolean fullOutput, Boolean stopOnFirstFail, float timeLimitMultiplier) throws IOException {
        System.out.println(name + "\n" + new String(new char[name.length()]).replace("\0", "-"));
        int correct = 0;
        int total = 0;
        Path path = Paths.get("resources", name);
        int longestTestNameLength = 0;
        String[] inputFiles1 = singleTestName.equals("") ? Objects.requireNonNull((new File(Paths.get(path.toString(),"in").toString())).list()) : new String[] { singleTestName + ".in" };
        ...
        for(String inputFile : inputFiles) {
            String testName = String.format("%1$-" + longestTestNameLength + "s", inputFile.replace(".in", ""));
            ...
            if(IsCustomValidated(name))
                correct += Test(f, testName, Paths.get(path.toString(), "in", inputFile), name, timeLimitMs, verbose, fullOutput) ? 1 : 0;
            else
                correct += Test(f, testName, Paths.get(path.toString(), "in", inputFile), Paths.get(path.toString(), "out", inputFile.replace(".in", ".out")), timeLimitMs, verbose, fullOutput) ? 1 : 0;
            ++total;
    ...
    private static boolean Test(Consumer<String[]> f, String testName, Path input, String name, long timeLimitMs, Boolean verbose, Boolean fullOutput) throws IOException {
        ...
        if(fullOutput)
            PrintFullOutput(testName, byteArrayOutputStream);
    ...
    private static boolean Test(Consumer<String[]> f, String testName, Path input, Path output, long timeLimitMs, Boolean verbose, Boolean fullOutput) throws IOException {
        List<Object> results = TestConsumer(f, input);
        ByteArrayOutputStream byteArrayOutputStream = (ByteArrayOutputStream) results.get(0);
        long runTimeMs = (long) results.get(1);
